...
def generate_mission(prompt):
    """Use the local Qwen LLM only to determine mission intent."""

    llm_prompt = (
        "You are a robot mission planner.\n"
        "The robot supports only inspection_loop.\n"
        "Return ONLY JSON.\n"
        "If the command is an inspection loop, return "
        '{"supported":true,"repeat":NUMBER}.\n'
        "Otherwise return "
        '{"supported":false,"reason":"unsupported command"}.\n'
        "Command: "
        + prompt
    )
    ...
    # The LLM does NOT control these values.
    mission = {
        'version': '1.0',
